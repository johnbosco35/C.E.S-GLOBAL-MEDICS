...
const PaymentDetailModal: React.FC<PaymentDetailModalProps> = ({ payment, isOpen, onClose, reloadPayments }) => {
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
          ...
          <div className="space-y-2 text-sm">
            <div>Session Number: <span className="font-mono">{payment.sessionNumber}</span></div>
            <div>Payment ID: <span className="font-mono">{payment._id}</span></div>
            <div>Status: <Badge className={getStatusColor(payment.paymentStatus)}>{payment.paymentStatus}</Badge></div>
            <div>Created: {payment.createdAt ? new Date(payment.createdAt).toLocaleString() : '-'}</div>
            <div>Updated: {payment.updatedAt ? new Date(payment.updatedAt).toLocaleString() : '-'}</div>
            <div>Total: <span className="font-semibold">₦{payment.totalAmount?.toLocaleString()}</span></div>
            <div>Shipping Fee: <span className="font-semibold">₦{payment.shippingFee?.toLocaleString()}</span></div>
            <div>Notes: {payment.notes || <span className="italic text-gray-400">None</span>}</div>
          </div>
          {/* Customer Info */}
          <div className="space-y-2 text-sm">
        ...
        {/* Payment Proof */}
        <div>
          <h3 className="font-semibold text-gray-900 dark:text-white mb-2">Proof of Payment</h3>
          {payment.paymentProof ? (
            <a href={payment.paymentProof} target="_blank" rel="noopener noreferrer">
              <img src={payment.paymentProof} alt="Payment Proof" className="w-48 h-48 object-contain border rounded-lg" />
            </a>
          ) : (
        ...
        {payment.paymentStatus === "submitted" && (
          <div className="flex flex-col md:flex-row gap-4 mt-6">
            ...
            <div className="flex flex-col gap-2">
              ...
              />
              <Button onClick={handleReject} disabled={loading || !rejectReason.trim()} className="bg-red-600 hover:bg-red-700 text-white">
